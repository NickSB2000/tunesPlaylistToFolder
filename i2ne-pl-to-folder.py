...
def get_drives():
  if os.name == 'nt':
    ...
    return glob.glob('/cygdrive/*')

ffmpegLocation = 'ffmpeg'
if os.name == 'nt':
  print("Looking for 'ffmpeg.exe' on your C: Drive, one moment...")
  ffmpegLocations = glob.glob('c:/**/ffmpeg.exe', recursive=True)
...
while True:  
  ...
  for item in trackList:
    ...
    if url_OBJ.scheme.lower() == 'file':
      ...
      if not ifexist:
        ...
        for drive in iterateOverDriveList:
          ...
          #searchresult = osCommand(searchCommand) 
    ...
    extrCmd = ffmpegLocation + ' -i "%s" "%s.jpg"' % (path_to_windows(destination), path_to_windows(destination))
    osCommand(extrCmd)
    print(" Removing All Tags..")
    ffCommand = ffmpegLocation + ' -i "%s" -vn -codec:a copy -map_metadata -1 "%s"' % (path_to_windows(destination), path_to_windows(newFileNamePath) )
    osCommand(ffCommand)
